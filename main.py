...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("-d", action="store_true", help="Scrape only todays matches")
...
def fetch_and_save_data(
    id: int, file_path: str, lock: threading.Lock, is_match_todays: bool
) -> None:
    try:
        ...
        data: dict = get_data(get_match_url(id), is_match_todays)
        data.update({"id": id})
